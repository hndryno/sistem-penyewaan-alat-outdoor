...
<div class="page-header">
    <h3 class="page-title">Transaksi Sudah Dibayar</h3>
</div>
<div class="row">
    ...
    <div class="card">
        ...
        <table class="table text-center table-responsive" id="order-listing">
            ...
            <tr>
                ...
                <td>
                    <form id="selesai-form-{{$transaksi->id}}" action="{{route('selesai',$transaksi->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
                        @if($transaksi->status_pembayaran == 0)
                        <button type="button" disabled class="btn btn-group btn-sm btn-outline-danger btn-icon-text mt-1" onclick="selesaiTransaksi({{$transaksi->id}})">Selesai </button>
                        @else
                        <button type="button" class="btn btn-group btn-sm btn-outline-primary btn-icon-text mt-1" onclick="selesaiTransaksi({{$transaksi->id}})">Selesai </button>
                        @endif
                    </form>
                </td>
            </tr>
